...
atom::vector3 vec3::sumCrime(const atom::vector3 a, const double k)
{
...
atom::vector3 vec3::normalCrime(const atom::vector3 a)
{
    ...
    {
        double sl = 0.01;
        double max = std::max(std::max(a.x, a.y), a.z);
        double x = a.x/max, y = a.y/max, z = a.z/max;
        return {x- sl, y-sl, z-sl}; 
    }
    return a;
}

atom::vector3 vec3::maxSum(const atom::vector3 a, const atom::vector3 b, const double limit)
{
    double x, y, z;
    bool bit = false;
    if((a.x + b.x <= limit) || (a.y + b.y <= limit) || (a.z + b.z <= limit))
        bit = true;

    x = (!bit)? a.x + b.x:std::max(a.x, b.x);
    y = (!bit)? a.y + b.y:std::max(a.y, b.y);
    z = (!bit)? a.z + b.z:std::max(a.z, b.z);
    return {x, y, z};
}
