...
void partialCast(spc::Camera c, spc::Scene s, ppm::img& image, int initial_line, int final_line, int id, int vida)
{
    // calc raycast time
    ...
    for(int i = initial_line; i <= final_line; i++)
        for(int j = 0; j < c.screen.hRes; j++)
            image[i][j] = s.getIntersectionColor(line3::fromPoints(c.originGlobal, c.scr[i][j]), vida);
...
void cast(const spc::Camera& c, const spc::Scene& s, ppm::img& image, int num_threads, int vida)
{
...
void rayPhong(const spc::Camera& c, const spc::Scene& s, int vida)
{
    std::ios_base::sync_with_stdio(false);

    ppm::img image(c.screen.vRes, std::vector<atom::rgb>(c.screen.hRes, {0,0,0}));

    cast(c, s, image, NUMTHREADS, vida);
...
void run()
{
    ...
    for(int x = 0; x < k; x++)
    {
        ...
        std::cin >> pt.a >> pt.d >> pt.s >> pt.n >> pt.rt.r >> pt.rt.t >> pt.rt.n;
        std::cout << "Constantes de phong são" << pt.a << " " << pt.d << " " << pt.s << " " << pt.n <<"\n\n";
        std::cout << "Constantes para RayTracing são" << pt.rt.r << " " << pt.rt.t << " " << pt.rt.n << " " <<"\n\n";
        char sep;
        std::cin >> sep;
